...
type SelectComponentProps = React.DetailedHTMLProps<
  ...
  HTMLInputElement
> & {label?: string,data?:selectItems[],onSelected?:(value:string|number|undefined)=>void};
...
const SelectComponent: React.FC<SelectComponentProps> = (props) => {
  ...
  const Filter = (search_value: string) => {
    setDatas(data.filter(data => data.label.toString().toLowerCase().includes(search_value.toLowerCase().trim())));
  }
  ...
  return (<div className="component-ui-select">
    ...
    <div className={showItems ? "component-ui-select-items" : "hide"} >
      <input type="text" name="search" className="component-ui-select-items-search" placeholder="Search ..." onChange={(e)=>Filter(e.target.value)}/>
      {datas.map((value,index)=><option className="component-ui-select-items-span" value={index} key={index} onClick={(e)=>HandleSelected(e)}>{value.label}</option>)}
    </div>
